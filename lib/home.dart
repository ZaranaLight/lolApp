...
class _MyHomeState extends State<MyHome> {
  ...
  getPostData() async {
    ...
    sdf.then((value) async {
      if (value.isEmpty) {
        print('insertTodo================456${value}');
        postL.addAll(value);
      } else {
        print('updateTodo================456${value}');
        postL = value;
  ...
  Widget build(BuildContext context) {
    print('postL length===========${postL.length}');
    print('postL length===========${postL}');
    // print('postL length===========${jsonDecode(postL[0]['data'])}');
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  SizedBox(
                    height: Get.height * .8,
                    child: homecontroller.isApplyLoading
                        ? Expanded(
                      ...
                      onRefresh: () async {
                        ...
                        if (homecontroller.pageIndex == 0 &&
                            postL[0]['data'].isEmpty) {
                          result = await homecontroller.getPostdata(
                              isRefresh: true);
                        }
                      ...
                      onLoading: () async {
                        print('omLoading------------');
                        final result =
                        await homecontroller.getPostdata();
                        if (result == true) {
                          homecontroller.postRefreshController
                              .loadComplete();
                        } else {
                      ...
                      child: ListView.builder(
                        itemCount: postL[0]['data'].length,
                        shrinkWrap: true,
                        itemBuilder:
                            (BuildContext context, int index) {
                          // print(
                          //     'yaa---${homecontroller.postList.length}');
                          var tempindex = index + 1;
                          final post =jsonDecode( postL[0]['data']);
print('post check===============${post}');
// print('post check===============${post['user_name']}');

                          // dynamic shareValue = post["share"].toString();
                        ...
                        // : homecontroller.postList.length == 0
                        //     ? Container(
                        //         child: Center(
                        //             child: Text(
                        //           'NO DATA',
