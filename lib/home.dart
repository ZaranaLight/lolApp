...
List postOfflineList= [];
  getOfflineData() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();

    var temp;
    temp = prefs.getString('offline_post_data') ?? '';
    print('temp===================1${temp}');
    print('temp===================3${jsonDecode(temp)}');
    postOfflineList = jsonDecode(temp);
    print('postofflineDecode====${postOfflineList}');
  }
  ...
  getPostData() async {
    ...
    sdf.then((value) async {
      if (value.isEmpty) {
        print('insertTodo================456${value[0]['data']}');
        postL.addAll(value);
        prefs.setString('offline_post_data', value[0]['data']);
      } else {
        print('updateTodo================456${value[0]['data']}');
          ...
          builder: (context, AsyncSnapshot<ConnectivityResult> snapshot) {
            ...
            if (snapshot.hasData) {
              ...
              if (result == ConnectivityResult.mobile) {
                ...
                print(
                    ' homecontroller.postList===${homecontroller.postList.runtimeType}');
                return
                //   SafeArea(
                //   child: Center(
                //       child: Column(
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          children: [
                            ...
                            // SizedBox(
                            //   height: Get.height * .8,
                            //   child: postOfflineList.length == 0
                            //       ? Container(
                            //     child: Center(child: CircularProgressIndicator()),
                            //   )
                            //       :
                            ...
                            //   //   controller: scrollController,
                            //   //   addAutomaticKeepAlives: true,
                            //   //   itemCount: postOfflineList.length,
                            //   //   shrinkWrap: true,
                            //   //   itemBuilder: (BuildContext context, int index) {
                            //   //     var tempindex = index + 1;
                            //   //     final post = postOfflineList[index];
                            //   //     String shareValue = post["share"].toString();
                            //   //     return Container(
                            //   //       // height: 380,
                            //   //       margin: const EdgeInsets.only(bottom: 15),
